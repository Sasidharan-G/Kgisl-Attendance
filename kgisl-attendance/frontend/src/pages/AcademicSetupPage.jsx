...
const empty = { name: '', department: 'Computer Applications', programme: 'MCA', semester: 1, academicYear: '2026-2027', mentorId: '', completionDate: '' };
...
export default function AcademicSetupPage() {
  ...
  const load = async () => { const [batchData, subjectData, roomData, facultyData] = await Promise.all([listBatches(), listSubjects(), listRooms(), listFaculty()]); setBatches(batchData); setSubjects(subjectData); setRooms(roomData); setFaculties(facultyData); };
  useEffect(() => { load(); }, []);

  async function submit(e) {
    e.preventDefault(); setSaving(true); setError('');
    try {
      const payload = { ...form, semester: Number(form.semester), mentorId: form.mentorId || null, completionDate: form.completionDate ? new Date(`${form.completionDate}T23:59:59`).toISOString() : null };
  ...
  return <div className="flex min-h-screen bg-ink-950"><Sidebar/><main className="min-w-0 flex-1 pb-10"><TopBar connected/><div className="mx-auto mt-6 max-w-6xl px-6">
    ...
    <form onSubmit={submit} className="mb-6 grid gap-3 rounded-2xl border border-ink-border bg-ink-850/60 p-5 md:grid-cols-3">
      ...
      <label className="space-y-1 text-xs text-slate-400"><span>Section mentor</span><select required value={form.mentorId} onChange={(e) => setForm({ ...form, mentorId: e.target.value })} className="w-full rounded-lg border border-ink-border bg-ink-900 px-3 py-2 text-sm text-white"><option value="">Select faculty mentor</option>{faculties.map((faculty) => <option key={faculty.id} value={faculty.id}>{faculty.name} · {faculty.email}</option>)}</select></label>
      {field('completionDate', 'Batch completion date', { type: 'date' })}
