...
const empty = { name: '', department: 'Computer Applications', programme: 'MCA', semester: 1, academicYear: '2026-2027', mentorId: '' };
...
export default function AcademicSetupPage() {
  ...
  const load = async () => { const [batchData, subjectData, roomData, facultyData] = await Promise.all([listBatches(), listSubjects(), listRooms(), listFaculty()]); setBatches(batchData); setSubjects(subjectData); setRooms(roomData); setFaculty(facultyData.filter((item) => item.isActive)); };
  useEffect(() => { load(); }, []);

  async function submit(e) {
    e.preventDefault(); setSaving(true); setError('');
    try {
      const payload = { ...form, semester: Number(form.semester), mentorId: form.mentorId || null };
  ...
  function edit(row) { setEditing(row.id); setForm({ name: row.name, department: row.department, programme: row.programme, semester: row.semester, academicYear: row.academicYear, mentorId: row.mentorId || '' }); }
  ...
  return <div className="flex min-h-screen bg-ink-950"><Sidebar/><main className="min-w-0 flex-1 pb-10"><TopBar connected/><div className="mx-auto mt-6 max-w-6xl px-6">
    ...
    <form onSubmit={submit} className="mb-6 grid gap-3 rounded-2xl border border-ink-border bg-ink-850/60 p-5 md:grid-cols-3">
      {field('name', 'Class / Section name', { placeholder: 'MCA - II Year A' })}{field('department', 'Department')}{field('programme', 'Programme')}{field('semester', 'Semester', { type: 'number', min: 1, max: 12 })}{field('academicYear', 'Academic year', { pattern: '\\d{4}-\\d{4}', placeholder: '2026-2027' })}<label className="space-y-1 text-xs text-slate-400"><span>Section mentor</span><select value={form.mentorId} onChange={(e) => setForm({ ...form, mentorId: e.target.value })} className="w-full rounded-lg border border-ink-border bg-ink-900 px-3 py-2 text-sm text-white"><option value="">No mentor assigned</option>{faculty.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
