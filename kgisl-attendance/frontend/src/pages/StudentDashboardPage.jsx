...
import { CalendarCheck, Clock3, ScanLine, ShieldAlert } from 'lucide-react';
...
export default function StudentDashboardPage() {
  ...
  return <div className="student-command-layout text-slate-200"><StudentSidebar/><div className="student-glass-page"><main className="mx-auto max-w-5xl">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm text-slate-400">Welcome back</p><h1 className="text-2xl font-bold text-white">{attendance?.student?.name || 'Student Dashboard'}</h1><p className="mt-1 text-sm text-slate-500">{attendance ? `${attendance.student.rollNo} - ${attendance.student.batchName}` : 'Loading your academic snapshot...'}</p></div><div className="flex items-center gap-3"><button onClick={() => navigate('/student/scan')} className="flex items-center gap-2 rounded-xl bg-signal-red px-4 py-2.5 text-sm font-bold text-white"><ScanLine size={17}/>Mark attendance</button></div></div>
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-2"><section className="rounded-2xl border border-ink-border bg-ink-850/60 p-5"><h2 className="font-bold text-white">Attendance alerts</h2>{shortage.length ? shortage.map((item) => <div key={item.code} className="mt-3 rounded-xl border border-red-400/20 bg-red-500/10 p-3"><p className="font-semibold text-red-200">{item.code} - {item.percentage}%</p><p className="mt-1 text-xs text-slate-300">Below 75%. Attend upcoming classes consistently.</p></div>) : <p className="mt-3 text-sm text-signal-green">No attendance shortage at the moment.</p>}<button onClick={() => navigate('/student/attendance')} className="mt-5 text-sm font-semibold text-signal-blue">View attendance history →</button></section><section className="rounded-2xl border border-ink-border bg-ink-850/60 p-5"><h2 className="font-bold text-white">Request updates</h2>{updates.length ? updates.map((item) => <div key={item.id} className="mt-3 flex justify-between rounded-xl bg-ink-900 px-3 py-2 text-sm"><span>{item.label}</span><span className={item.status === 'APPROVED' ? 'text-signal-green' : item.status === 'REJECTED' ? 'text-red-300' : 'text-signal-amber'}>{item.status}</span></div>) : <p className="mt-3 text-sm text-slate-500">No leave or correction updates yet.</p>}<div className="mt-5 flex gap-4"><button onClick={() => navigate('/student/leave')} className="text-sm font-semibold text-signal-blue">Leave / OD</button><button onClick={() => navigate('/student/attendance')} className="text-sm font-semibold text-signal-blue">Report missed attendance</button></div></section></div>
  </main></div></div>;
}
