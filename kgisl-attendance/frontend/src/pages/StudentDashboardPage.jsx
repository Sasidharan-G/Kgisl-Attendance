...
export default function StudentDashboardPage() {
  ...
  return <div className="student-glass-page text-slate-200"><main className="mx-auto max-w-5xl">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm text-slate-400">Welcome back</p><h1 className="text-2xl font-bold text-white">{attendance?.student?.name || 'Student Dashboard'}</h1><p className="mt-1 text-sm text-slate-500">{attendance ? `${attendance.student.rollNo} - ${attendance.student.batchName}` : 'Loading your academic snapshot...'}</p></div><div className="flex items-center gap-3"><NotificationBell /><button onClick={() => navigate('/student/scan')} className="flex items-center gap-2 rounded-xl bg-signal-red px-4 py-2.5 text-sm font-bold text-white"><ScanLine size={17}/>Mark attendance</button></div></div>
